...
var llmTask = task({
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    try {
      ...
      const modelToUse = payload.imageUrls?.length ? "meta-llama/llama-4-scout-17b-16e-instruct" : "llama-3.3-70b-versatile";
      ...
      if (payload.systemPrompt?.toLowerCase().includes("marketing") || payload.systemPrompt?.toLowerCase().includes("product")) {
      ...
      } else if (payload.systemPrompt?.toLowerCase().includes("tweet") || payload.systemPrompt?.toLowerCase().includes("social")) {
