...
console.log('TRIGGER KEY prefix:', process.env.TRIGGER_SECRET_KEY?.slice(0, 10))
  const parsed = schema.safeParse(await req.json())
  if (!parsed.success) return NextResponse.json({ error: parsed.error }, { status: 400 })

  const { imageUrl, xPercent, yPercent, widthPercent, heightPercent, nodeId, workflowRunId } = parsed.data
